    ...
    for f in range(1, args.k_fold_cross_val + 1):
        ...
        for e in range(args.epochs):
            ...
            for i in tqdm(range(0, num_train_points, args.batch_size)):
                optimizer.zero_grad()
                if args.classification_type == "binary":
                    y_batch, y_hat, loss, prot_seq_batch = step(model, *dataset.get_batch(i, args.batch_size, f, "Train"))
                    ...
                    y_batch, y_hat_mm = step(model, *dataset.get_batch(i, args.batch_size, f, "Train"))
                    true_vals.extend(y_batch.tolist())
            ...
            if args.classification_type == "binary":
                ...
                visualization_df.to_csv("output/charts_trial/visualization_train_info.csv",index=False)

                scores_df = pd.DataFrame({'label':true_vals,'score':pred_vals})
                model.blm.add_model(f'epoch_{e}', scores_df)
                model.blm.plot_roc(model_names=[f'epoch_{e}'],params={"save":True,"prefix":f"output/charts_trial/fold_{f}_epoch_{e}_"})
            else:
                ...
                blm.plot_roc(chart_types=[1,2], params={"legloc":4, "addsz":False, "save":True, "prefix":f"output/charts_trial/fold_{f}_epoch_{e}_"})
        ...
        predictions = []
        ground_truth = []
        print(f"Validating Fold {f}")
        for i in tqdm(range(0, num_val_points, args.batch_size)):
            if args.classification_type == "binary":
                y_batch, y_hat, loss, prot_seq_batch = step(model, *dataset.get_batch(i, args.batch_size, f, "Val"))
                true_vals.extend(y_batch.tolist())
                pred_vals.extend(y_hat.tolist())
                predictions.append(y_hat.tolist())
                ground_truth.append(y_batch.tolist())
                ...
                scores_df = pd.DataFrame({'label':true_vals,'score':pred_vals})
                model.blm.add_model(f'epoch_{e}', scores_df)
                model.blm.plot_roc(model_names=[f'epoch_{e}'],params={"save":True,"prefix":f"output/charts_trial/fold_{f}_val_"})
            else:
                y_batch, y_hat_mm = step(model, *dataset.get_batch(i, args.batch_size, f, "Val"))
                true_vals.extend(y_batch.tolist())
                pred_vals.append(y_hat_mm)
